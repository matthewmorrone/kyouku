...
enum FuriganaAttributedTextBuilder {
    ...
    static func computeStage2(
        ...
        if baseSpans == nil {
            let session = EmbeddingAccessSession()
    ...
    static func project(
        ...
        for semantic in semanticSpans {
            ...
            let segments = FuriganaProjector.project(spanText: spanText, reading: reading, spanRange: semantic.range)

            for segment in segments {
                guard segment.reading.isEmpty == false else { continue }
                mutable.addAttribute(.furiganaAnnotation, value: true, range: segment.range)
