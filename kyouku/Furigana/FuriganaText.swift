...
struct FuriganaText: UIViewRepresentable {
    ...
    var textInsets: UIEdgeInsets = FuriganaText.defaultInsets
    var bottomOverlayHeight: CGFloat = 0
    var annotationVisibility: FuriganaAnnotationVisibility = .visible
    ...
    func makeUIView(context: Context) -> TokenOverlayTextView {
        ...
        if wrapLines == false {
            ...
            textView.textContainer.size = CGSize(width: FuriganaTextConstants.noWrapContainerWidth, height: CGFloat.greatestFiniteMagnitude)
        }
    ...
    func updateUIView(_ uiView: TokenOverlayTextView, context: Context) {
        ...
        uiView.furiganaAnnotationVisibility = annotationVisibility
        ...
        if wrapLines == false, uiView.textContainer.size.width < FuriganaTextConstants.noWrapContainerWidth {
            uiView.textContainer.size = CGSize(width: FuriganaTextConstants.noWrapContainerWidth, height: CGFloat.greatestFiniteMagnitude)
        }
        ...
        let resolvedRubyBaselineGap = max(0, furiganaBaselineGap)
        ...
        let rubyHeadroom = max(
            ...
            max(
                ...
                FuriganaText.requiredVerticalHeadroomForRuby(
                    ...
                    furiganaBaselineGap: resolvedRubyBaselineGap
                )
            )
        )
        ...
        if rubyMetricsEnabled {
            ...
            let rubyHorizontalInset = FuriganaText.requiredHorizontalInsetForRubyOverhang(
                in: attributed,
                baseFont: baseFont,
                defaultRubyFontSize: defaultRubyFontSize
            )
        ...
        uiView.furiganaBaselineGap = resolvedRubyBaselineGap
        ...
        renderHasher.combine(Int((max(0, furiganaBaselineGap) * 1000).rounded(.toNearestOrEven)))
        ...
        renderHasher.combine(furiganaHorizontalAlignment == .leading ? 1 : 0)
        ...
        if shouldReapplyAttributedText {
            ...
            if tokenOverlays.isEmpty == false {
                FuriganaTextProcessing.applyTokenColors(tokenOverlays, to: mutable)
            }

            if customizedRanges.isEmpty == false {
                FuriganaTextProcessing.applyCustomizationHighlights(customizedRanges, to: mutable)
            }

            let processed = FuriganaTextProcessing.applyAnnotationVisibility(annotationVisibility, to: mutable)

            let (displayText, indexMap) = FuriganaTextProcessing.applyRubyWidthPaddingAroundRunsIfNeeded(
                to: processed,
                baseFont: baseFont,
                defaultRubyFontSize: defaultRubyFontSize,
        ...
        uiView.furiganaHorizontalAlignment = furiganaHorizontalAlignment
    ...
    class Coordinator: NSObject, UITextViewDelegate {
        var stateProvider: (() -> FuriganaContextMenuState?)?
        var actionHandler: ((FuriganaContextMenuAction) -> Void)?
    ...
    static func requiredVerticalHeadroomForRuby(
        ...
        FuriganaTextProcessing.requiredVerticalHeadroomForRuby(
            in: attributed,
            baseFont: baseFont,
            defaultRubyFontSize: defaultRubyFontSize,
    ...
    static func requiredHorizontalInsetForRubyOverhang(
        ...
        FuriganaTextProcessing.requiredHorizontalInsetForRubyOverhang(
            in: attributed,
            baseFont: baseFont,
            defaultRubyFontSize: defaultRubyFontSize
        )
    ...
    static func measureTypographicSize(_ attributed: NSAttributedString) -> CGSize {
        FuriganaTextProcessing.measureTypographicSize(attributed)
    }
