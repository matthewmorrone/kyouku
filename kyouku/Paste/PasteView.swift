...
struct PasteView: View {
    ...
    @State var savedWordOverlays: [FuriganaText.TokenOverlay] = []
    ...
    private var inlineContextMenuState: FuriganaContextMenuState? {
        guard let selection = tokenSelection else { return nil }
        return FuriganaContextMenuState(
    ...
    @AppStorage(FuriganaKnownWordsSettings.modeKey) private var knownWordFuriganaModeRaw: String = FuriganaKnownWordsSettings.defaultModeRawValue
    @AppStorage(FuriganaKnownWordsSettings.scoreThresholdKey) private var knownWordFuriganaScoreThreshold: Double = FuriganaKnownWordsSettings.defaultScoreThreshold
    @AppStorage(FuriganaKnownWordsSettings.minimumReviewsKey) private var knownWordFuriganaMinimumReviews: Int = FuriganaKnownWordsSettings.defaultMinimumReviews
    ...
    private var editorColumn: some View {
        ...
        let extraOverlays: [FuriganaText.TokenOverlay] = {
            var overlays: [FuriganaText.TokenOverlay] = incrementalLookupEnabled ? savedWordOverlays : []
        ...
        let spanSelectionHandler: ((FuriganaSpanSelection?) -> Void)? = { selection in
            if incrementalLookupEnabled {
        ...
        let contextMenuStateProvider: (() -> FuriganaContextMenuState?)?
        if incrementalLookupEnabled {
        ...
        let contextMenuActionHandler: ((FuriganaContextMenuAction) -> Void)?
        if incrementalLookupEnabled {
        ...
        let viewMetricsContext = FuriganaText.ViewMetricsContext(
    ...
    private func noteSpeechOverlay(for text: String) -> FuriganaText.TokenOverlay? {
    ...
    private func tokenOverlay(for text: String, range: NSRange) -> FuriganaText.TokenOverlay? {
        ...
        return FuriganaText.TokenOverlay(range: range, color: color)
    }
    ...
    private func handleInlineSpanSelection(_ selection: FuriganaSpanSelection?) {
        ...
        // FuriganaText.TokenOverlayTextView.handleInspectionTap → spanSelectionHandler → PasteView.handleInlineSpanSelection
    ...
    private func handleContextMenuAction(_ action: FuriganaContextMenuAction) {
        switch action {
        case .mergeLeft:
    ...
    private func makeFuriganaTask(token: Int, recomputeSpans: Bool) -> (() async -> Void)? {
        ...
        // Any heavy work inside rendering/ruby layout that happens on the main thread should be flagged separately (see FuriganaText).
        ...
        let knownWordSurfaceKeys: Set<String> = {
            let mode = FuriganaKnownWordsMode(rawValue: knownWordFuriganaModeRaw) ?? .off
