...
struct SettingsView: View {
    ...
    init() {
        ...
        if defaults.bool(forKey: "didMigrateReadingRubyBaselineGap") == false {
            ...
            if old != 0 {
                defaults.set(max(0, 0.5 + old), forKey: "readingFuriganaBaselineGap")
                defaults.set(0.0, forKey: "readingRubyBaselineGapAdjustment")
            } else if defaults.object(forKey: "readingFuriganaBaselineGap") == nil {
                defaults.set(0.5, forKey: "readingFuriganaBaselineGap")
            }
        }
    }
    ...
    private var textAppearanceSection: some View {
        Section("Reading Appearance") {
            ...
            HStack {
                Text("Ruby Gap")
                Spacer()
                Text(String(format: "%.2f pt", readingFuriganaBaselineGap))
                    .foregroundStyle(.secondary)
            }
            Slider(
                value: $readingFuriganaBaselineGap,
                in: 0.0...12.0,
                step: 0.25
            )
            HStack {
    ...
    private var furiganaBehaviorSection: some View {
        ...
        return Section("Furigana") {
            Picker("Hide furigana", selection: $knownWordFuriganaModeRaw) {
                ForEach(FuriganaKnownWordsMode.allCases) { m in
